...
Server::Server(int port, std::string password, IrcClients *ircClients, CommandFactory *commandFactory)
	 : portNb(port),
	  password(password),
	  ipAddress(IP_BIND),
	  hostName(HOSTNAME),
...
{
	int fd = socket(AF_INET, SOCK_STREAM, 0); // Creating the Socket: The socket is created using the socket() function with the parameters AF_INET (IPv4), SOCK_STREAM (TCP), and 0 (default protocol for TCP).
    if (fd < 0)
	{
	...
	/*
	Setting SO_REUSEADDR: The socket option SO_REUSEADDR is set using the setsockopt() function. This allows the socket to reuse the local
	address if it is already in use. The setsockopt() function is called with the parameters SOL_SOCKET (socket level), SO_REUSEADDR
	(socket option), and a pointer to the value reuse (set to 1).
	*/
	int enable = 1;
	...
	/*
	O_NONBLOCK is a flag used to set a file descriptor, such as a socket, to non-blocking mode. When a file descriptor is in non-blocking mode,
	operations on it, like reading or writing, will return immediately without waiting for the operation to complete. This is particularly useful
	in network programming, where you don't want your application to be blocked waiting for I/O operations to complete.
	*/
	if (fcntl(fd, F_SETFL, O_NONBLOCK) < 0) {
		close (fd);
		std::cerr << "ERROR! Error setting socket to non-blocking mode!" << std::endl;
		exit (1);
	}

	// Bind the Socket: The socket is bound to the configured address and port using the bind() function.
 ...
 // Listen for Connections: The socket is set to listen for incoming connections using the listen() function, with a backlog of MAXCOUNT pending connections.
...
void Server::acceptConnection()
{
	...
	if (clientSocket < 0) {
		std::cerr << " Connection can not be established with new client" << std::endl;
		return ;
	}
	...
	std::cout <<"Client successfully connected on socket fd: " << clientSocket << std::endl;
	sendResponse(clientSocket,  "Register the yourself is mandatory to use the server, by using: PASS, NICK and USER\r\n");
}
...
void Server::readMessage(int i)
{
	...
	if (bytesRecv <= 0) {
		if (bytesRecv == 0) // only when close terminal
			std::cerr << "Client disconnected from socket fd: " << clientFd << std::endl;
		else
	...
	std::cout << "Received: " << bytesRecv << " bytes Raw msg:" << std::string(buffer) << std::endl;
	...
	if (currentClient != NULL)
	{
		...
		{
			std::cout << "Message ready to be processed: " << currentClient->getEntireMessage() << std::endl;
			IrcCommandParser parser = IrcCommandParser(std::string(currentClient->getEntireMessage()));
